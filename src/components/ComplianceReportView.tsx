    ...
    <div className="flex flex-col h-full overflow-auto">
      ...
      <div className="px-6 py-5 border-b">
        ...
        <p className="text-xs text-muted-foreground">
          21 CFR Part 11 audit trail summary for CSR-2026-0042
        </p>
      </div>
      ...
      <div className="p-6 space-y-6 max-w-5xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <SummaryCard
            ...
            highlight={verifiedClaims.length === allClaims.length}
          />
          <SummaryCard
            label="Flagged"
            value={flaggedClaims.length}
            icon={<Flag className="h-4 w-4 text-destructive" />}
          />
          <SummaryCard
            label="Tier 3 Progress"
            value={`${tier3Verified.length}/${tier3Claims.length}`}
            icon={<AlertTriangle className="h-4 w-4 text-amber-accent" />}
            highlight={tier3Verified.length === tier3Claims.length}
          />
        </div>
        ...
        <div className="rounded-lg border bg-card p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-data text-foreground">{new Date().toLocaleString()}</span>
            </div>
          </div>
...
function ClaimSection({ title, icon, claims, auditorName }: {
  title: string; icon: React.ReactNode; claims: ClaimData[]; auditorName?: string;
}) {
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
        {icon} {title} ({claims.length})
      </h3>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-primary text-primary-foreground">
              ...
              {auditorName && (
                <th className="px-3 py-2 text-left font-medium text-[10px] uppercase tracking-wider">Auditor</th>
              )}
            </tr>
          </thead>
          <tbody>
            {claims.map((claim, i) => (
              <tr key={claim.id} className={i % 2 === 0 ? "bg-card" : "bg-muted/30"}>
                ...
                {auditorName && (
                  <td className="px-3 py-2 text-[10px]">{auditorName}</td>
                )}
              </tr>
            ))}
          </tbody>
