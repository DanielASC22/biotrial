...
const ComplianceReportModal = () => {
  const { claims, sessionHash, activeStudy } = useStudy();
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head><title>Bio-Trial Auditor — Regulatory Report</title><style>
      ...
      <div class="footer"><p>Generated by Bio-Trial Auditor v1.0 • ${new Date().toISOString()}</p>
      <p class="hash">Session Integrity Hash: ${sessionHash}</p></div></body></html>`);
    win.document.close();
    win.print();
  };
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader className="flex-row items-center justify-between pr-8">
          <DialogTitle className="text-base flex items-center gap-2">
            <FileBarChart className="h-4 w-4" /> Regulatory Compliance Report
          </DialogTitle>
        ...
        <div className="flex-1 overflow-auto pr-2" ref={printRef}>
          <div className="mb-6">
            ...
            <p className="text-xs text-muted-foreground font-data">Generated: {new Date().toLocaleString()}</p>
          </div>

          <div className="grid grid-cols-3 gap-3 mb-6">
            <StatCard label="Verified" count={verifiedClaims.length} total={allClaims.length} icon={<ShieldCheck className="h-4 w-4 text-compliance" />} />
            <StatCard label="Flagged" count={flaggedClaims.length} total={allClaims.length} icon={<Flag className="h-4 w-4 text-destructive" />} />
            <StatCard label="Pending" count={pendingClaims.length} total={allClaims.length} icon={<AlertTriangle className="h-4 w-4 text-amber-accent" />} />
          </div>
          ...
          {verifiedClaims.length > 0 && (
            <div className="mb-6">
              <h2 className="text-sm font-semibold mb-3 flex items-center gap-1.5">
                <ShieldCheck className="h-4 w-4 text-compliance" /> Verified Claims ({verifiedClaims.length})
              </h2>
              <ClaimTable claims={verifiedClaims} />
            </div>
          )}
          {flaggedClaims.length > 0 && (
            <div className="mb-6">
              <h2 className="text-sm font-semibold mb-3 flex items-center gap-1.5">
                <Flag className="h-4 w-4 text-destructive" /> Flagged Claims ({flaggedClaims.length})
              </h2>
              <ClaimTable claims={flaggedClaims} />
            </div>
          )}
          {pendingClaims.length > 0 && (
            <div className="mb-6">
              <h2 className="text-sm font-semibold mb-3 flex items-center gap-1.5">
                <AlertTriangle className="h-4 w-4 text-amber-accent" /> Pending Review ({pendingClaims.length})
              </h2>
              <ClaimTable claims={pendingClaims} />
            </div>
          )}
          ...
          <p className="font-data text-[10px] text-muted-foreground break-all">SHA-256: {sessionHash}</p>
        </div>
      </DialogContent>
    </Dialog>
  );
...
function StatCard({ label, count, total, icon }: { label: string; count: number; total: number; icon: React.ReactNode }) {
  return (
...
function ClaimTable({ claims }: { claims: ClaimData[] }) {
  return (
